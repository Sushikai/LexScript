...
def create_user(username: str, password: str, email: str | None = None,
    ...
    """创建用户。skip_validation=True 跳过密码强度校验(用于预置默认账户)。"""
...
def ensure_default_admin():
    """创建 root + root2 两个默认账户(密码均为 123456),数据完全隔离。
    仅首次启动时执行,已存在则跳过。
    """
    from app.config import (
        DEFAULT_ADMIN_USERNAME, DEFAULT_ADMIN_PASSWORD,
        DEFAULT_SECONDARY_USERNAME, DEFAULT_SECONDARY_PASSWORD,
    )

    def _ensure_one(username, password, role="admin"):
        if get_by_username(username):
            return None
        return create_user(
            username=username,
    ...
    results = []
    for uname, pwd in [(DEFAULT_ADMIN_USERNAME, DEFAULT_ADMIN_PASSWORD),
                        (DEFAULT_SECONDARY_USERNAME, DEFAULT_SECONDARY_PASSWORD)]:
        u = _ensure_one(uname, pwd, role="admin")
        if u:
            results.append(u)
