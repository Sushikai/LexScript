...
def init_schema(migrations_dir: Path):
    """按序执行 migrations/*.sql + Python 级迁移。"""
    conn = get_conn()
    ...
    conn.commit()
    _run_python_migrations(conn)


def _run_python_migrations(conn: sqlite3.Connection):
    """Python 级迁移：owner_user_id 列、数据加密等。"""
    ...
    tables = [
        "files", "file_chunks", "case_folders",
        "chat_sessions", "chat_messages",
        "documents", "templates", "document_corrections",
    ]
    for tbl in tables:
        try:
            conn.execute(f"ALTER TABLE {tbl} ADD COLUMN owner_user_id INTEGER REFERENCES users(id)")
        except Exception:
            pass
    # 现有数据归 user_id=1 (root)
    for tbl in tables:
        try:
            conn.execute(f"UPDATE {tbl} SET owner_user_id = 1 WHERE owner_user_id IS NULL")
        except Exception:
            pass
    conn.commit()